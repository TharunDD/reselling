...
const Ord = () => {
  ...
  const hao = (product) => {
    ...
    if (confirmed) {
      ...
      axios
        .delete(`http://3.110.128.177:3000/b/product/de/${product._id}`)
        .then((response) => {
  ...
  useEffect(() => {
    axios
      .get(`http://3.110.128.177:3000/b/product/bu/${us.uid}`)
        ...
        {users.map((product) => (
          <div className="cart-item" key={product._id}>
            <div className="cart-product">
              <img src={`http://3.110.128.177:3000/${product.image}`} alt="product" />
              <div>
                <h3>{product.model}</h3>
                <p>{product.desc}</p>
              </div>
