...
const ShoppingCart = () => {
  ...
  const handleopenpay=(data)=>{
    const options={
      ...
      "handler": function (response){
        ...
        axios.post(`http://3.110.128.177:3000/verify`,{response:response}).then((res)=>{
          toast.success("order completed");
           axios
        .post(`http://3.110.128.177:3000/b/product/cart/${u.uid}`, cart.Cartitems) // Sending the array directly
        .then((response) => {
  ...
  const handleBuyClick = (us) => {
    ...
    axios.post(`http://3.110.128.177:3000/orders`,_data).then((res)=>{
      if(res){
